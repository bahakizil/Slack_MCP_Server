...
class TavilyMCPServer:
    ...
    def _register_tools(self) -> None:
        ...
        def search_web(
            ...
            """
            if not self.api_key:
                raise McpError("Tavily API key not configured")
        ...
        def search_news(
            ...
            try:
                ...
                raise McpError(f"News search failed: {e}") from e
        ...
        def research_topic(
            ...
            # Main topic search
            main_search = self.search_web(
                query=topic, max_results=5, search_depth="advanced"
            )
            ...
            if focus_areas:
                for area in focus_areas[:3]:  # Limit to 3 areas
                    focus_search = self.search_web(
                        query=f"{topic} {area}", max_results=3
                    )
